...
interface VendorMemberTaskScopeProps {
  ...
  tasks: Task[]
  onScopeChange: (memberId: string, taskIds: string[]) => Promise<void>
}
...
export function VendorMemberTaskScope({
  ...
  const [selectedIds, setSelectedIds] = useState<Set<string>>(
    new Set(member.vendor_task_ids ?? [])
  )
  ...
  async function handleSave() {
    ...
    try {
      ...
      console.error('Failed to update vendor task scope:', err)
        ...
        <div className="space-y-2 p-1">
          {tasks.map((task) => (
            <div key={task.id} className="flex items-center gap-2">
              <input
                type="checkbox"
                id={`scope-${member.id}-${task.id}`}
                checked={selectedIds.has(task.id)}
                onChange={() => handleToggle(task.id)}
                className="h-4 w-4 rounded border-gray-300"
              />
              <Label htmlFor={`scope-${member.id}-${task.id}`} className="font-normal cursor-pointer">
                {task.name}
              </Label>
            </div>
          ))}
